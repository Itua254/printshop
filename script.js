...
// --- SHOPPING CART STATE ---
let cart = JSON.parse(localStorage.getItem('arshrozy_cart')) || [];
...
// 2. Add To Cart
function addToCart(category, totalId, details) {
    const totalEl = document.getElementById(totalId);
    // Parse "KES 1,500.00" -> 1500.00
    const rawPrice = totalEl.innerText.replace(/[^0-9.]/g, '');
    const price = parseFloat(rawPrice);

    if (price <= 0) return alert("Please select a valid quantity.");
    ...
    saveCart();
    renderCartUI();
    toggleCartModal(true); // Open cart to show visual confirmation
}

// 3. Cart Management
function saveCart() {
    localStorage.setItem('arshrozy_cart', JSON.stringify(cart));
    updateCartCount();
}
...
// 4. Send Order via WhatsApp (Checkout)
function checkout() {
    ...
    cart.forEach((item, index) => {
        ...
        msg += `   Price: ${formatMoney(item.price)}\n\n`;
